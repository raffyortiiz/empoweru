...
$query = "SELECT * FROM applicant WHERE status ='Pending'";
...
while($row = mysqli_fetch_array($result))
{
 ...
 $sub_array[] = '<div class="update" data-id="'.$row["appID"].'" data-column="">' . $row["status"] . '</div>';
  $sub_array[] = '<div data-id="'.$row["appID"].'" data-column=""><a href="../uploads/'.$row["filename"].'" download>Download</a></div>';
 $sub_array[] = '<button type="button" name="approve" class="btn btn-success btn-xs approve" appID="'.$row["appID"].'">Approve</button>';
 $sub_array[] = '<button type="button" name="reject" class="btn btn-danger btn-xs reject" appID="'.$row["appID"].'">Remove</button>
 ';
